          ...
          return (
            <Link key={item.href} href={item.href} className="flex-1">
              <div className="flex flex-col items-center gap-0.5 py-1 relative">
                {isActive && (
                  <span className="absolute -top-1 w-5 h-0.5 rounded-full bg-emerald-500" />
                )}
                <item.icon
                  className={`h-5 w-5 transition-colors ${isActive ? 'text-emerald-600' : 'text-slate-500'}`}
                />
                <span
                  className={`text-[10px] font-medium transition-colors ${isActive ? 'text-emerald-600' : 'text-slate-500'}`}
                >
                  {item.label}
                </span>
              </div>
